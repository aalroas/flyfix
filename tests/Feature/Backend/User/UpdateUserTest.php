...
class UpdateUserTest extends TestCase
{
    ...
    public function a_user_can_be_updated()
    {
        ...
        $this->patch("/admin/auth/user/{$user->id}", [
            ...
            'email' => 'john@example.com',
            'roles' => [
                Role::whereName(config('kodatik.access.role.admin'))->first()->id,
            ],
        ]);
        ...
        $this->assertDatabaseHas('model_has_roles', [
            'role_id' => Role::whereName(config('kodatik.access.role.admin'))->first()->id,
    ...
    public function only_the_master_admin_can_edit_themselves()
    {
        ...
        $otherAdmin->assignRole(config('kodatik.access.role.admin'));
